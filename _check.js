  ...
  if(!cart.length){el.innerHTML='<div class="empty-state">Cart is empty</div>';}
    ...
    el.innerHTML=cart.map(function(c,idx){
      ...
      if(c.sellByPill&&c.pillsPerUnit>0){
        ...
        controls='<div class="qty-controls"><button onclick="changeCartQty(\''+c.productId+'\',-1)">-</button><span>'+c.qty+(c.unit?' '+esc(c.unit):'')+'</span><button onclick="changeCartQty(\''+c.productId+'\',1)">+</button></div>';
      }
...
var UNIT_OPTIONS = [
  '','tablet','capsule','caplet','softgel','lozenge','suppository',
  'strip','blister','sachet','packet','ampoule','vial',
  'bottle','can','jar','tube','spray','inhaler','dropper',
  'box','carton','pack','bag',
  'ml','L','g','mg','kg',
  'pcs','unit','dose','pair','set'
];
function unitSelectHtml(selectedUnit){
  return '<select class="pl-unit" style="width:90px;padding:4px;font-size:12px;border:1px solid var(--line);border-radius:6px;">'+
    UNIT_OPTIONS.map(function(u){
      return '<option value="'+u+'"'+(u===(selectedUnit||'')?'selected':'')+'>'+(u||'-- unit --')+'</option>';
    }).join('')+
  '</select>';
}
function addPurchaseLine(){
  ...
  div.innerHTML=
    '<div class="li-top" style="display:flex;gap:10px;margin-bottom:8px;align-items:center;">'+
      ...
      '<button type="button" class="li-remove" onclick="this.closest(\'.line-item-row\').remove();recalcPurchaseTotals();">X</button>'+
    '</div>'+
    ...
    '<div class="li-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:10px;align-items:center;">'+
      '<div class="pl-qty-wrapper">'+
        '<div style="display:flex;gap:4px;align-items:center;">'+
          '<input class="pl-qty" type="number" min="0" placeholder="Qty" style="width:70px;padding:4px;">'+
          unitSelectHtml('')+
        '</div>'+
      '</div>'+
    ...
    if(p) {
      ...
      if(p.SellByPill) {
        ...
        qtyWrap.innerHTML = '<div style="display:flex;gap:4px;align-items:center;">'+
          '<input class="pl-qty" type="number" min="0" placeholder="Units" style="width:60px;padding:4px;">'+
          unitSelectHtml(p.Unit)+
          '<span style="font-size:11px;color:var(--ink-soft);">+</span>'+
          '<input class="pl-pills" type="number" min="0" placeholder="Pills" style="width:60px;padding:4px;">'+
          '<span style="font-size:11px;color:var(--ink-soft);">pills</span>'+
          '</div>';
      } else {
        info.textContent = 'Standard product - ' + (p.Strength||'') + (p.DosageForm?' '+p.DosageForm:'');
        qtyWrap.innerHTML = '<div style="display:flex;gap:4px;align-items:center;">'+
          '<input class="pl-qty" type="number" min="0" placeholder="Qty" style="width:70px;padding:4px;">'+
          unitSelectHtml(p.Unit)+
          '</div>';
