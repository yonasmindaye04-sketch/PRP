...
function enterApp(){
  ...
  document.getElementById('add-product-btn').style.display = can('PERM_CREATE_PRODUCT') ? 'inline-block' : 'none';
...
function renderCart(){
  ...
  if(!cart.length){el.innerHTML='<div class="empty-state">Cart is empty</div>';}
    ...
    el.innerHTML=cart.map(function(c,idx){
      var marginOpts=marginPresets.map(function(m){return '<option value="'+m+'"'+(c.marginUsed==m?' selected':'')+'>'+m+'%</option>';}).join('');
      ...
      return '<div class="cart-item" style="flex-direction:column;align-items:stretch;gap:4px;">'+
        ...
        '<div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">'+
          ...
          '<select style="font-size:11px;padding:2px 4px;border:1px solid var(--line);border-radius:4px;" onchange="changeCartMargin(\''+c.productId+'\',this.value)">'+marginOpts+'</select>'+
          '<span style="font-size:10.5px;color:var(--ink-soft);">'+money(c.marginUsed)+'% margin</span>'+
        '</div>'+
      '</div>';
    }).join('');
  }
...
function addPurchaseLine(){
  ...
  div.innerHTML=
    '<div class="li-top">'+
      '<select class="pl-product">'+allProducts.map(function(p){return '<option value="'+p.ProductID+'">'+esc(p.ProductName)+'</option>';}).join('')+'</select>'+
      '<button type="button" class="li-remove" onclick="this.closest(\'.line-item-row\').remove();recalcPurchaseTotals();">✕</button>'+
    '</div>'+
    '<div class="li-grid">'+
      '<input class="pl-qty" type="text" placeholder="Qty">'+
      '<input class="pl-cost" type="text" placeholder="Purchase price">'+
      '<input class="pl-price" type="text" placeholder="Selling price">'+
...
document.getElementById('purchase-lines').addEventListener('input', function(e){
  if(e.target.classList.contains('pl-qty')||e.target.classList.contains('pl-cost')) recalcPurchaseTotals();
});
...
document.getElementById('purchase-modal-save').addEventListener('click',function(){
  ...
  document.querySelectorAll('#purchase-lines .line-item-row').forEach(function(row){
    var qty=Number(row.querySelector('.pl-qty').value);
    if(qty>0){
      items.push({
        productId:row.querySelector('.pl-product').value, quantity:qty,
